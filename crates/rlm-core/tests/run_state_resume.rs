...
use rlm_core::ports::{LanguageModel, LanguageModelCompleteOptions, RunStateStorePort};
...
#[tokio::test]
async fn partial_run_restart_resume_completes_without_rerunning_finished_nodes() {
    let dir = tempfile::tempdir().expect("tempdir");
    let run_state_dir = dir.path().join(".planning/runs");
    fs::create_dir_all(&run_state_dir).expect("run state dir");
    let run_id = "run-resume-test";
