...
Executable pass/fail commands for Phases 114–120 teardown executors. Cross-reference inventory: `113-AUDIT.md`.
...
**Delete:** `src/application/control-server/`, `src/cli/run-modes/ui.ts`, `scripts/parity/compare-runtimes.mjs`, `tests/integration/control-server-fixtures.test.ts`, TS portions of `check:parity`.

**Keep:** `tests/fixtures/control-server/*.json`, `ui/`, Rust control server, `crates/rlm-core/tests/control_server_fixtures.rs`.
...
3. `test ! -d src/application/control-server` — exit 0 (directory removed)
4. `test ! -f tests/integration/control-server-fixtures.test.ts` — exit 0 (TS fixture test removed)
5. Verify `check:parity` no longer boots TS server — target shape after Phase 114:
...
**Delete:** `src/index.ts`, `src/cli/`, `src/runtime/`, `package.json` direct Node bin paths (`rlm:node`, `dev`, `start`).

**Keep:** `scripts/rlm-runtime.mjs` (simplified to Rust-only dispatch), `ui/`, `crates/`.

**Gate commands:**

1. `cargo test -p rlm-core` — exit 0
2. `npm run rlm -- --help` — exit 0; invokes Rust CLI via dispatcher
3. `test ! -f src/index.ts` — exit 0
4. `cargo check -p rlm-tauri` — exit 0 (Tauri smoke); manual: `npm run tauri:dev` checkpoint

**Rollback:** `git checkout HEAD~1 -- src/index.ts src/cli/ src/runtime/ package.json`
...
**Delete:** `src/application/`, `tests/application/`.

**Keep:** `ui/`, `crates/rlm-core/src/application/`, fixture data.

**Gate commands:**

1. `cargo test -p rlm-core` — exit 0
2. `test ! -d src/application` — exit 0
3. `npm run rlm -- ask --help` — exit 0 (Rust ask path)
...
**Keep:** `crates/rlm-core/src/domain/`, `crates/rlm-core/src/ports/`, `ui/`, `scripts/`.
...
**Keep:** `tests/ui/`, `tests/fixtures/`, `tests/depcruise/` (until Phase 119), `ui/`, `crates/`.

**Gate commands:**

1. `cargo test -p rlm-core` — exit 0
2. `test ! -d src/adapters && test ! -d src/plugins` — exit 0
3. `test ! -d tests/domain` — exit 0 (mirrored domain tests removed in 117; verify no stragglers)

**Rollback:** `git checkout HEAD~1 -- src/adapters/ src/plugins/ tests/adapters/ tests/plugins/ tests/runtime/`
...
**Preconditions:** Phase 118 gates passed; entire `src/` absent.

**Delete:** TS build deps, depcruise, Node-specific scripts, `typecheck` (or no-op).

**Keep:** Vite/UI deps, `npm run build:ui`, `npm run check:rust`, packaging scripts, RAM gates.
...
**Rollback:** `git checkout HEAD~1 -- package.json package-lock.json tsconfig.json .dependency-cruiser.js`

---

## Phase 120: Constrained Ollama Tool Envelope (Rust)

**Scope:** Post-cutover tool-call hardening via Ollama JSON-schema envelope (Option A from research doc).

**Preconditions:** Phase 119 gates passed; Rust-only runtime.

**Delete:** None (additive hardening in Rust).

**Keep:** Existing two-phase Ollama path when envelope mode off.
...
**Rollback:** `git checkout HEAD~1 -- crates/rlm-core/src/adapters/`
...
- **Target `check:parity` after 114:** Rust golden fixtures only (no TS build, no TS server boot)
...
- `113-AUDIT.md` — TS-only path inventory with teardown phase mapping
- `113-GATES.md` — this document
- `.planning/notes/rust-only-runtime-migration-decisions.md` — canonical teardown policy
- `crates/rlm-core/tests/control_server_fixtures.rs` — Rust golden fixture test
- `tests/fixtures/control-server/` — shared fixture JSON data
